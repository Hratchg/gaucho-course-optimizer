...
- [x] SEC-1 — Purge the 13 MB scraped-comment dump from public git history (High, S)
- [x] BUG-5 — Apply the Bayesian adjustment in the API and unify scoring paths (High, M)
- [x] BUG-7 — Backfill course titles from the UCSB API (High, M)
- [x] DATA-2 — Handle truncated Nexus names; stop scoring unmatched professors as neutral (High, L)
...
- [x] UX-4 — Add source attribution and data-freshness timestamps (Medium, S)
...
- [x] SEC-2 — Rate-limit the public search and professors endpoints (Medium, M)
...
- [x] ARCH-2 — Move `queries.py` out of `dashboard/`; delete the Streamlit app (Medium, M)
- [x] PERF-2 — Paginate the professors endpoint and defer heavy per-professor fields (Medium, M)
